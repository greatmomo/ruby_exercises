...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      it 'is your choice of beverage' do
        expect(horchata.type).to eq('horchata')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(horchata.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:hot_chocolate) { described_class.new('hot chocolate', 8) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(hot_chocolate).not_to be_full
      end
    end
  end
end
