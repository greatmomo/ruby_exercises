...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 12' do
      it 'returns 6' do
        previous_step = 12
        random_number_game = MagicSeven.new(6)
        result = random_number_game.subtract_random_number(previous_step)
        expect(result).to eq(6)
      end
    end
  end
  ...
  describe '#play' do
    context 'when the random number is 1' do
      ...
      it 'will return 7' do
        generation = MagicSeven.new(1)
        result = generation.play
