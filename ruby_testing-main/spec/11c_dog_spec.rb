...
describe Dog do
  ...
  subject(:nimbus) { described_class.new('Nimbus', 'goldendoodle', 'white') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when a dog is asked to talk' do
    it 'says WOOF!' do
      expect(nimbus.talk).to eq('WOOF!')
    end
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(nimbus).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(nimbus).to be_sleeping
  end
end
